package st.addon.ui.provider.views;
...
import st.plugin.quantum.entity.QuantumNetwork;
import st.plugin.quantum.provider.QuantumMapTile;
import st.plugin.quantum.provider.QuantumWorld;
import st.addon.block_io.provider.QuantumInterface;
...
@Provider
public class QuantumNetworkDashUI extends LayerProvider {
	@Require(cls = Tab.class)
	Tab tabs;
	@Require(cls = Title.class)
	Title title;
	...
	public class component {
		public StBuilding building;
		public StBlock block;
		
		public component(StBuilding b) {
			var view = tabs.new component("quantum-network-dash", Styles.fullDialog) {
				//检查进度条
				@Override
				public void hide() {
					super.hide();
			...
			view.slot("@st.quantum-network-tab-network", t -> {
				...
				if (!quantumMap.isSector) {
					t.labelWrap("@st.quantum-network-control-not-sector").left();
					t.row();
					return;
				}
				var ts = quantumWorld.team(building.team.id);
				renderCapacity(t, ts);
				if (!isInterface) {
					renderItemList(t, ts);
					renderLiquidList(t, ts);
		...
		public void renderList(renderListConfig config) {
			config.t.add(title.new component(config.title)).expandX().left();
			config.t.row();
			...
			if (config.exportMap == null) {
				for (var v : config.list) {
					...
					var lab = new Table();
					lab.image((TextureRegion) v.icon);
		...
		public ArrayList<valueInfo> getItemsList(QuantumNetwork ts, boolean isCore) {
			...
			for (var i : Vars.content.items()) {
				...
				data.add(new valueInfo() {{
					...
					name = i.name;
					max = quantumWorld.team(building.team.id).itemCapability;
					icon = i.uiIcon;
		...
		public ArrayList<valueInfo> getLiquidsList(QuantumNetwork ts, boolean isCore) {
			...
			for (var i : Vars.content.liquids()) {
				...
				data.add(new valueInfo() {{
					lozName = i.localizedName;
					name = i.name;
					max = quantumWorld.team(building.team.id).liquidCapability;
					icon = i.uiIcon;
		...
		public void renderCapacity(Table t, QuantumNetwork ts) {
			...
			t.add(title.new component("@capacity")).expandX().left();
			t.row();
