...
public class T1Units extends LayerProvider {
	...
	public void run() {
		...
		//量子建造机
		{
			unitAttack.injectWeapon(量子建造机, unitAttack.pos_光粒(1, 1), bullet.弹药_机枪.build(50, items.超导体, 28, 5, 0.5f)
		...
		//量子挖矿机
		{
			unitAttack.injectWeapon(量子挖矿机, unitAttack.pos_光粒(1, 1), bullet.弹药_机枪.build(26, items.超导体, 26, 5, 0.5f)
		...
		//量子特工队
		{
			unitAttack.injectWeapon(量子特工队, unitAttack.pos_激光炮(0, 5), bullet.弹药_机枪.build(100, items.超导体, 26, 10, 0.8f)
				.frag(bullet.弹片_狙击, items.金元素, 1, 0.2f, 5, 0.5f)
				.bullet());
			unitAttack.injectWeapon(
				量子特工队,
				unitAttack.pos_导弹(0, 0).top(false),
				bullet.弹药_导弹.build(100, items.超导体, 26, 0.5f, 0.5f)
		...
		//量子电锯人
		{
			unitAttack.injectWeapon(量子电锯人,
				unitAttack.pos_机枪(0, 0),
				bullet.弹药_导弹
					.build(250, items.土元素, 40, 1, 1f)
					.frag(bullet.弹片_爆炸, items.土元素, 1, 0.5f, 12, 1)
					.frag(bullet.弹片_元素狙击, items.土元素, 1, 0.1f, 4, 1)
					.bullet()
			);
			unitAttack.injectWeapon(量子电锯人, unitAttack.pos_激光炮(0, 0), bullet.弹药_激光.build(500, items.木元素, 20, 1, 2.8f)
				.bullet());
			unitAttack.injectWeapon(量子电锯人, unitAttack.pos_机枪(-12, 6), bullet.弹药_元素.build(300, items.火元素, 35, 12, 0.5f)
				.frag(bullet.弹片_元素狙击, items.金元素, 1, 0.2f, 5, 0.5f)
				.bullet());
			unitAttack.injectWeapon(量子电锯人, unitAttack.pos_机枪(12, 6), bullet.弹药_元素.build(300, items.水元素, 35, 6, 0.5f)
				.frag(bullet.弹片_元素狙击, items.金元素, 1, 0.2f, 5, 0.5f)
				.bullet());
		...
		//原始人单位厂
		{
			...
			原始人单位构造厂.plans.add(new UnitFactory.UnitPlan(量子特工队, 60 * 20, ItemStack.with(items.纳米碳管, 200, items.超导体, 150, Items.silicon, 150, Items.metaglass, 50, Items.thorium, 130)));
			原始人单位构造厂.plans.add(new UnitFactory.UnitPlan(量子电锯人, 60 * 30, ItemStack.with(items.纳米碳管, 800, items.超导体, 600, Items.silicon, 450, Items.metaglass, 550, Items.plastanium, 500, Items.thorium, 130, Items.titanium, 150)));
