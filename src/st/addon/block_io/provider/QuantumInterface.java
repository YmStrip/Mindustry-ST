...
import st.plugin.quantum.entity.QuantumRequest;

import java.util.Map;
import java.util.concurrent.atomic.AtomicBoolean;

public class QuantumInterface extends StBlock {
	
	public TextureRegion topRegion;
	public TextureRegion mainRegion;
	
	public QuantumInterface(EntityStore store, String name) {
		super(store, name);
		hasPower = true;
		size = 8;
		update = true;
	...
	public class QuantumInterfaceBuild extends StBuilding {
		...
		public void use() {
			...
			var taken = store.quantumWorld.take(this.team.id + "", require, store.quantumMap.sector);
			//v0(false);
			//add-data
